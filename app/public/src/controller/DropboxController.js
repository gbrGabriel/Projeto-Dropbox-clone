class DropboxController {
    ...
    uploadTask(files) {
        ...
        [...files].forEach(file => {
            ...
            promises.push(new Promise((resolve, reject) => {
                ...
                formDat.append('input-file', file);
    ...
    uploadProgress(event, file) {
        ...
        let timelief = ((time - porcent) * timeSpent) / porcent;

        this.progressBarEl.style.width = `${porcent}%`;

        this.nameFileEl.innerHTML = file.name;
        this.timeLeftEl.innerHTML = this.formatTime(timelief);

    }
    formatTime(duration) {
        let seg = duration / 1000;
        ...
        if (hor >= 1) {
            ...
            return `${parseInt(hor)} horas, ${parseInt(min)} minutos e ${parseInt(seg)} segundos`;

        } else if (min >= 1) {

            return `${parseInt(min)} minutos e ${parseInt(seg)} segundos`;

        } else {

            return `${parseInt(seg)} segundos`;
        }
